...
export const postMessageThunk = createAsyncThunk<
    // Optimistic-append payload — the message entity returned by the API so
    // the fulfilled reducer can splice it into state.Message.messages without
    // waiting for the socket echo. Sandbox's legacy backend had the same
    // socket gap, so relying on realtime alone meant a page reload between
    // every send.
    any,
    PostMessageParams,
    AsyncThunkOptions
>(
    ...
    async (params, { rejectWithValue }) => {
        try {
            ...
            const response = await apiFetch(postConversationMessage.api, postConversationMessage.method, requestBody);
...
>(
    ...
    async (eventPayload, { getState, rejectWithValue, dispatch }) => {
        try {
            ...
            // Dedup by message id — the optimistic path in postMessageThunk.fulfilled
            // already splices the agent's own send into state, so the socket echo
            // would otherwise arrive as a duplicate.
            const incomingId = (eventPayload as any).id || (eventPayload as any)._id;
            const alreadyPresent = incomingId
                ? state.Message.messages.some((m: any) => (m.id || m._id) === incomingId)
                : false;
            const messages =
                state.TeamConversation.teamConversation?.conversation_id === conversation_id && !alreadyPresent
                    ? [
    ...
    extraReducers: (builder) => {
        ...
        builder
            ...
            .addCase(postMessageThunk.fulfilled, (state, action) => {
                ...
                // Optimistic append: splice the server-returned message into
                // state.Message.messages so the user sees their own send
                // immediately. The socket echo (if/when it arrives) is idempotent
                // via id dedup in appendMessageThunk.
                const msg = action.payload as any;
                if (msg && msg.id) {
                    const alreadyPresent = state.messages.some((m: any) => (m.id || m._id) === msg.id);
                    if (!alreadyPresent) {
