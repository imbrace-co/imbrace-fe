...
import { fetchNotificationsThunk, pushNotification } from './notification';
import type {
...
>('TeamConversation/join', async (params, { getState, rejectWithValue, dispatch }) => {
    try {
        ...
        // Update conversation list cache so the Join → Leave button flips immediately
        queryClient.setQueriesData<{ pages?: TeamConversationsQueryData[] }>(
                ...
                return {
                    ...
                    pages: prev.pages.map((page) => ({
                        ...
                        data: page.data.map((conv) =>
                            conv.id === teamConvId || conv._id === teamConvId
                                ? { ...conv, is_joined: true, is_agent_joined: true }
                                : conv,
                        ),
                    })),
                };
...
export const leaveTeamConversationThunk = createAsyncThunk<JoinTeamConversationThunkPayload, { teamConvId: string }, AsyncThunkOptions>(
    ...
    async (params, { getState, rejectWithValue, dispatch }) => {
        try {
            ...
            // Update conversation list cache so the Leave → Join button flips immediately
            queryClient.setQueriesData<{ pages?: TeamConversationsQueryData[] }>(
                { queryKey: ['teamConversations'] },
                (prev) => {
                    if (!prev?.pages) return prev;
