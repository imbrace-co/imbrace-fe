...
export const fetchOrganizationListThunk = createAsyncThunk<FetchOrganizationPayload, PaginationParams, AsyncThunkOptions>(
    ...
    async (params, { dispatch, getState, rejectWithValue }) => {
        try {
            const { limit = 100, skip = 0, is_active = true } = params;
            const state = getState();

            const getOrgApi = getOrganization;
            const getOrganizationApi = getOrgApi.api(limit, skip, is_active);
            const response = await apiFetch<API.PaginatedResponse<API.Organization[]>>(getOrganizationApi, getOrgApi.method);
...
export const { updateOrganizationList } = organizationSlice.actions;
