import LoanTimeline from '@/pages/Databoards/components/RecordDetail/Activities/Timeline';
...
const dateRangeOptions = [
    { text: t('contact_activity_last_7_days'), value: 'last_7_days' },
    { text: t('contact_activity_this_month'), value: 'this_month' },
    { text: t('contact_activity_last_month'), value: 'last_month' },
    { text: t('contact_activity_all_time'), value: 'all_time' },
]
...
const fetchActivities = async ({ queryKey }: { queryKey: (string | undefined)[] }) => {
    ...
    const { data } = await apiFetch<{ items?: API.ConversationActivity[]; data?: API.ConversationActivity[] }>(
        api,
        getContactActivitiesByConversationId.method
    );
...
export default function Activities({ userId, frameLess = false, inModal = false }: { userId?: string, frameLess?: boolean, inModal?: boolean }) {
                ...
                <Illustration
                    ...
                    description={
                        <Box sx={{ width: '328px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                            <Typography variant="SubHeading2">{t('contacts_no_activities_found')}</Typography>
                            <Typography variant="Caption">{t('contacts_no_activities_found_for_selected_date_range')}</Typography>
                        </Box>
                    }
                />
            );
    ...
    const renderDate = () => {
        return dateRangeOptions.find((option) => option.value === selectedOption)?.text.toLowerCase();
    };
            ...
            <Space direction="vertical" align="stretch" size={16}>
                <Space direction="horizontal" align="center" justify="between" size={16}>
                    <Space size={4} direction="horizontal" align="center">
                        <Typography variant="BodyBold">{activities.length} {t('contact_activity_conversations')}</Typography>
                        <Typography variant="Body" style={{ color: 'var(--color-secondary-3)' }}>
                            {t('contact_activity_in')} {renderDate()}
                        </Typography>
                    </Space>
                    <FieldSelect
    ...
    return (
        <div className={styles.container} style={{ height: inModal ? '80%' : 'auto' }}>
            <Space size={16} direction="vertical" align="stretch" className={clsx(styles.inner, frameLess && styles.frameLess)}>
                {!frameLess && <Typography variant="SubHeading2">{t('activities_in_conversation')}</Typography>}
                {render()}
            </Space>
        </div>
    );
