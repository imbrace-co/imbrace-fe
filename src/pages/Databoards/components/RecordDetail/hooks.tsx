...
import { getBoardRecord, postBoardFile, postBoardRecord, postBoardUpload, putBoardRecord } from '@/services/api/crm';
import { ImbraceFileUpload } from '@/services/axios';
import apiFetch from '@/services/axios/handler';

import type { RecordValue } from './types';
import type { ParentRecordContext } from './modal';
...
const useRecordDetailHelper = ({
    ...
    const createRecord = useMutation({
        mutationFn: async (params: { boardId: string; fields: { board_field_id: string; value: unknown }[] }) => {
            const { data } = await apiFetch<{ data: API.BoardItem } | API.BoardItem>(postBoardRecord.api(params.boardId), postBoardRecord.method, {
                fields: params.fields,
            });
            // The API wraps the record under `data` (e.g. `{ data: item }`); unwrap so the returned object exposes id/fields at the top level.
            const createdRecord = (data as { data: API.BoardItem }).data ?? (data as API.BoardItem);
    ...
    const updateRecord = useMutation({
        mutationFn: async (params: { boardId: string; recordId: string; data: { key: string; value: unknown }[] }) => {
            const { data } = await apiFetch<{ data: API.BoardItem } | API.BoardItem>(
            ...
            // The API wraps the record under `data` (e.g. `{ data: item }`); unwrap so the returned object exposes id/fields at the top level.
            const updatedRecord = (data as { data: API.BoardItem }).data ?? (data as API.BoardItem);
                    ...
                    if (currentField?.type === 'Attachment' && Array.isArray(newValue)) {
                        ...
                        if ([...formData.entries()].length > 0) {
                            ...
                            newValue = (newValue as AttachmentValue[]).map((item) => {
                                const targetName = item.data?.name?.includes('.') ? item.data.name.split('.')[0] : item.data?.name;
                                const targetData = data.find((d) => d.name === targetName);
                                return {
                                    type: item.type,
                                    data: {
                                        name: item.data?.name || '',
                    ...
                    const updateData = await Promise.all(
                        board.fields.map(async (field) => {
                            ...
                            try {
                                ...
                                if (field.type === 'Attachment' && Array.isArray(newValue)) {
                                    ...
                                    if ([...formData.entries()].length > 0) {
                                        ...
                                        newValue = (newValue as AttachmentValue[]).map((item) => {
                                            ...
                                            const nameWithoutExtension = itemName?.includes('.') ? itemName.substring(0, itemName.lastIndexOf('.')) : itemName || '';
                                ...
                                if (field.type === 'TableInTable' && newValue) {
                                    return newValue as { board_field_id: string; value: unknown };
                                }
                                return {
                    ...
                    const updateData = await Promise.all(
                        board.fields.map(async (field) => {
                            ...
                            try {
                                ...
                                if (field.type === 'TableInTable' && newValue && typeof newValue === 'object' && 'value' in newValue) {
    ...
    const onAppendChildRowToParent = useCallback(
        async (parentContext: ParentRecordContext, fieldsData: Record<string, RecordValue>) => {
            if (!board?.fields) {
                throw new Error('missing child board fields');
            }
            const { parentBoardId, parentItemId, parentFieldId } = parentContext;

            const newRow: Record<string, unknown> = {};
            board.fields.forEach((field) => {
                let value = fieldsData[field._id];
                if (field.type === 'Country' && typeof value === 'object' && value) {
                    value = (value as API.CountryValue).country_code;
                }
                if (field.type === 'Assignee' && typeof value === 'object' && value) {
                    value = (value as API.AssigneeValue)._id;
                }
                if (field.type === 'MultipleAssignee' && Array.isArray(value)) {
                    value = (value as Array<API.AssigneeValue | string>).map((item) =>
                        typeof item === 'object' && item && '_id' in item ? item._id : item,
                    );
                }
                newRow[field._id] = value ?? null;
            });

            try {
                const { data: parentItem } = await apiFetch<API.BoardItem>(
                    getBoardRecord.api(parentBoardId, parentItemId),
                    getBoardRecord.method,
                );
                const currentValue = parentItem?.fields?.[parentFieldId];
                const currentArr = (Array.isArray(currentValue) ? currentValue : []) as unknown as Array<Record<string, unknown>>;
                // Preserve existing rows as references; new row must omit _id so backend creates it.
                const preservedRows = currentArr.map((row) => {
                    if (row && typeof row === 'object') {
                        const id = (row as { _id?: unknown; board_item_id?: unknown })._id ??
                            (row as { board_item_id?: unknown }).board_item_id;
                ...
                await apiFetch<API.BoardItem>(
                    putBoardRecord.api(parentBoardId, parentItemId),
                    putBoardRecord.method,
                    {
                        data: [{ key: parentFieldId, value: [...preservedRows, newRow] }],
                    },
                );
                await refresh();
                notify({
                    type: 'success',
                    message: t('databoard_child_row_added'),
                });
            } catch (error) {
                const err = error as AxiosError<{ message?: string }>;
                notify({
                    type: 'error',
                    message: err.response?.data?.message || t('databoard_child_row_add_error'),
                });
                throw error;
            }
        },
        [board, refresh, notify, t],
    );
