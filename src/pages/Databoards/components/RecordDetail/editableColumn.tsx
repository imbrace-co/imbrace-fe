import type { channelIconMapping, DatePickerRef, DateTimePickerRef, FieldTextProps, Option, SelectRef, TimePickerRef } from '@imbrace/ui';
import { Checkbox, EllipsisText, FieldText, Icon, IconButton, Space, Tooltip, Typography } from '@imbrace/ui';
...
import type { AttachmentValue, ColumnValue } from '@/components/FlexibleTable/types';
...
import { TableInTableDetail } from './TableInTableDetail';
...
interface FieldColumnProps {
    ...
    updateData?: (value: { fieldId: string; value: RecordValue; recordId: string }) => Promise<API.BoardItem | undefined>;
    readonly?: boolean;
...
export const FieldColumn = forwardRef<HTMLDivElement, FieldColumnProps>((props, ref) => {
    ...
    // Calculate text rows
    const calculateTextRows = (text: string): number => {
        if (!text) return 0;
        const lines = text.split('\n');
        let totalRows = 0;

        lines.forEach((line) => {
            // Calculate how many rows this line will take based on character count
            // Assuming average character width and container width
            const estimatedRows = Math.ceil(line.length / 80); // Rough estimation
            totalRows += Math.max(1, estimatedRows);
        });

        return totalRows;
    };

    // Check if text has more than 7 rows
    const textValue = typeof value === 'string' ? value : '';
    const hasMoreThan7Rows = calculateTextRows(textValue) > 7;
                ...
                return (
                    <Fields
                        ...
                        fieldProps={{
                            ...
                            //     }),
                            ...(fieldType === 'LongText' && {
                                minRows: hasMoreThan7Rows ? 14 : 7,
                                maxRows: hasMoreThan7Rows ? 14 : 7,
                                sx: {
    ...
    const renderFormattedValue = () => {
                ...
                if (fieldType === 'MultipleSelection') {
                    if (Array.isArray(currentValue) && currentValue.length > 0) {
                        return (
                            <Space size={8}>
                                {currentValue
                                    .filter((enumValue) => enumValue !== undefined && value !== null)
                                    .map((enumValue, index) => (
                                        <Chip
                                            key={`${fieldId}-${index}`}
                                            sx={{
        ...
        if (fieldType === 'Attachment') {
            return (
                <Attachment
                    value={currentValue as API.AttachmentValue[]}
                    fieldId={fieldId}
                    onUpdate={async (newValue?: AttachmentValue[]) => {
                        ...
                        onChange?.(newValue as API.RecordValue);
                    }}
                />
            );
        }
        ...
        if (fieldType === 'Notes') {
            return (
                <Notes
                    type="list"
                    notes={(currentValue as API.NotesValue[]) || []}
                    // containerStyle={{
                    //     paddingRight: '12px',
                    // }}
                    {...notesProps}
        ...
        if (fieldType === 'Currency') {
            ...
            return (
                <EllipsisText
                    text={`${
                        currencyData?.currency_code || meta?.defaultCurrencyCode
                            ? Intl.NumberFormat('en-US', {
                                  style: 'currency',
                                  currency: currencyData?.currency_code || meta?.defaultCurrencyCode,
        ...
        //     const richContent = currentValue as unknown as {
        //         subject: string;
        //         content: { content: string; files: [] };
        //     };
        //     const senderEmailField = table._getColumnDefs().find((columnDef) => columnDef.meta?.fieldData?.name === 'Outbound Source');

        //     return (
        //         <EmailContentPreview
        //             {...richContent}
        //             email={senderEmailField && senderEmailField.id ? (row.original[senderEmailField.id] as string) || '' : ''}
        //         />
        //     );
        // }
        if (
    ...
    const renderContent = () => {
        // Special handling for TableInTable - should display in both edit and readonly modes
        if (fieldType === 'TableInTable') {
            console.log('Rendering TableInTable field:', {
                field,
                recordId: record._id,
                recordIdAlt: record.id,
                hasRecordId: !!record._id,
                editing,
                record,
            });

            if (!field) {
                console.log('TableInTable missing field:', { field: !!field });
                return <Typography style={{ color: 'var(--color-light-4)' }}>—</Typography>;
            }

            return (
                <div style={{ padding: '12px 0', border: 'none' }}>
                    <TableInTableDetail
                        field={field}
        ...
        if ((editing && editable) || record.id === 'new') {
            return (
                <div
                    ...
                    style={{
                        ...(fieldType === 'LongText' && {
                            height: hasMoreThan7Rows ? '332px' : '166px',
                        }),
                    }}
                >
                    {renderField()}
            ...
            >
                <div
                    ...
                    style={{
                        ...
                        ...(fieldType === 'LongText' && {
                            height: hasMoreThan7Rows ? '328px' : '164px',
                            padding: '12px',
                        }),
                        ...(fieldType === 'Notes' && {
                            minHeight: '164px',
                ...
                >
                    <Scrollbars
                        autoHide
                        autoHeight
                        autoHeightMax={fieldType === 'Notes' ? 488 : fieldType === 'LongText' && hasMoreThan7Rows ? 300 : 140}
                    >
                        {renderFormattedValue()}
                    </Scrollbars>
                </div>

                {(fieldType === 'MultipleSelection' || fieldType === 'LongText' || fieldType === 'Attachment') && !readonly && (
                    <div
    ...
    return (
        <div
            ...
            className={clsx(
                ...
                bordered === 'always' && fieldType !== 'TableInTable' && styles.bordered,
                readonly && styles.readonly,
                focus && fieldType !== 'TableInTable' && styles.focus,
            )}
        >
...
const EditableColumn = (
    ...
    const { fieldId, record, updateData, bordered = 'hover', readonly, editing: controlledEditing, disableClickAway, fieldType } = props;
...
export const EditableIdentifier = ({
    ...
    const onSubmit = async (data: { value?: string }) => {
        await update.mutateAsync({ fieldId, value: data.value, recordId });
    };

    const handleEdit = async () => {
        if (update.isPending || value === getValues('value')) {
            setEditing(false);
            reset({ value: value });
            return;
        }
