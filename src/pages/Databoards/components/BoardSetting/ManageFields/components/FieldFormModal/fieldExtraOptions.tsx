...
interface Props {
    ...
    disabled?: boolean;
    /** Document-models style: leading number instead of the drag icon, no floating label. */
    numbered?: boolean;
}
...
const OptionContainer = ({
    ...
    return (
        <>
            <div ref={containerRef} className={`${styles.fieldContainer}${numbered ? ` ${styles.numberedField}` : ''}`}>
                <div className={styles.dragContainer}>
                    {numbered ? (
                        // Leading position number doubles as the drag handle (keeps reorder working).
                        <span
                            ...
                            style={{
                                ...
                                color: 'var(--color-light-5)',
                                fontSize: 14,
                                cursor: shouldDisabled ? 'default' : state.type === 'dragging' ? 'grabbing' : 'grab',
                            }}
                        >
                            {index + 1}
                        </span>
                ...
                createPortal(
                    ...
                    >
                        <div className={styles.fieldContainer}>
                            <DraggableOption control={control} boardField={boardField} remove={remove} index={index} state={state} />
                        </div>
                    </div>,
                    state.container,
                )}
...
const FieldExtraOptions = ({ boardType, boardName, containerRef, methods, boardField, disabled, numbered }: Props) => {
    const { t } = useTranslation();
    const { control } = methods;
    const fieldType = useWatch({ control, name: 'type' });
    const fieldName = useWatch({ control, name: 'name' });
    const [instanceId] = useState(() => Symbol('FieldExtraOptions'));
    const {
        fields = [],
        append,
        remove,
    ...
    const renderExtraOptions = () => {
        if (fieldType === 'MultipleSelection' || fieldType === 'SingleSelection' || fieldType === 'Priority' || fieldType === 'Origin') {
            const optionShouldDisable = (option: string, index: number) => {
                if (boardType === 'System') {
                    switch (boardName) {
                        case 'Email Outbound Records':
                            if (boardField?.is_default) {
                ...
                if (index + 1 > (boardField?.data?.length ?? 0)) {
                    return false;
                }
                if (disabled) {
                    return true;
                                    ...
                                    return (
                                        ...
                                        >
                                            <OptionContainer
                                                ...
                                                label={numbered ? '' : `${t('fields_management_row_option')} ${virtualRow.index + 1}${
                                                    virtualRow.index === 0 ? '*' : ''
                                                }`}
                                            />
                                        </Space>
        ...
        if ((fieldType === 'Phone' || fieldType === 'Currency') && FieldExtraSetting[fieldType]) {
                    ...
                    render={({ field: { onChange, value }, fieldState: { error } }) => {
                        switch (extraSettingField.type) {
                            ...
                            default:
                                return (
                                    <FieldSelect
                                        queryKey={['fieldExtra', extraSettingField.name]}
                                        label={`${t(extraSettingField.label)}${extraSettingField.required ? '*' : ''}`}
                                        fullWidth
                                        onChange={onChange}
                                        value={value}
                                        ...
                                        searchable
                                        searchFn={extraSettingField.searchFn}
                                    />
                                );
                        }
                    }}
