    ...
    const renderAction = () => {
        if (isEdit) {
            return (
                <Space style={{ marginTop: '32px' }} justify="between">
                    <Space>
                        <Button variant="link" onClick={onBack} text={t('segmentation_select_other_saved_segment')} />
                    </Space>
                    <Space size={16}>
                        <Button type="danger" variant="outlined" onClick={onDelete} text={t('delete')} />
                        <Button onClick={handleSubmit(onSubmitHandler)} text={t('update')} />
                    </Space>
                </Space>
            );
        }
        return (
            <Space style={{ marginTop: '32px' }} justify="end">
                <Button variant="link" onClick={onBack} sx={{ textTransform: 'uppercase', marginRight: '24px' }} text={t('back')} />
                <Button onClick={handleSubmit(onSubmitHandler)} text={t('apply')} />
            </Space>
        );
    };
                    ...
                    <Scrollbars autoHeight autoHeightMax={460} autoHide>
                        <Space size={8} direction="vertical" align="start" style={{ width: '100%' }}>
                            <Space size={16} direction="vertical" align="stretch" style={{ width: '100%' }}>
                                <Typography variant="BodyBold">{t('segmentation_conditions')}</Typography>
                            ...
                            <Space size={24}>
                                <Button
                                    ...
                                    text={t('segmentation_add_condition')}
                                    onClick={() => {
                                        append({
                                            operator: 'is',
