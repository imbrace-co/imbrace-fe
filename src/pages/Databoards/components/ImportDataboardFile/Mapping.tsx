...
// Normalize a CSV/Excel header cell before it is used as a mapping key.
// Handles files with a doubled UTF-8 BOM: PapaParse only strips one BOM, so the
// first column arrives as `﻿"Name"`. Because it no longer starts with `"`,
// PapaParse treats it as unquoted and keeps the literal quotes. Strip any leading
// BOMs, then the surrounding quotes, so the header matches what the backend's
// csv-parse produces (otherwise the mapped value is silently dropped).
const cleanCsvHeader = (header: string): string =>
    header
        .replace(/^﻿+/, '')
        .replace(/^"(.*)"$/, '$1')
        .trim();
...
const MappingData = ({
        ...
        return new Promise((resolve, reject) => {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    // NOTE: `sheet_to_json(..., { header: 1 })` can return a *sparse* array for the header row
                    // (DevTools shows this as `[empty, ...]`). `Array.prototype.map()` skips holes, so we must
                    // generate headers by index to ensure fallbacks apply.
                    const headerRow = (jsonData[0] as any[]) ?? [];
                    const headers = Array.from({ length: headerRow.length }, (_, index) => {
                        const h = headerRow[index];
                        const trimmed = h === undefined || h === null ? '' : cleanCsvHeader(h.toString());
                        return trimmed === '' ? `Header ${index + 1}` : trimmed;
                    });
                    console.log('getMappingData - headers', headers);
    ...
    const getMappingData = async () => {
        try {
            ...
            console.log('getMappingData - headers', headers);
            console.log('getMappingData - records', records);
            const boardFields = boardData?.fields || [];

            const mappingRows: MappingRow[] = headers.map((header) => {
                // Clean the header by removing BOM/surrounding quotes and trimming.
                // Idempotent with transformHeader above; kept as defense.
                const cleanHeader = cleanCsvHeader(header);
    ...
    const uploadDialogCloseRef = useRef<(() => void) | null>(null);

    const openUploadingDialog = useCallback(() => {
        dialog({
            title: '',
            ...
            actionsAlign: 'flex-start',
            content: ({ onClose }) => {
                uploadDialogCloseRef.current = onClose || null;
                return (
                    <Space direction="vertical" size={12} style={{ padding: '4px 0' }}>
                        <Space direction="horizontal" size={12} align="center">
                            <CircularProgress size={18} sx={{ color: 'var(--color-light-5)' }} />
                            <Typography variant="BodyBold">
                                {t('crm_import_uploading_desc', { defaultValue: 'Uploading… Please don’t close this dialog.' })}
                            </Typography>
                        </Space>
                        <Typography style={{ color: 'var(--color-light-6)' }}>
                            {t('crm_import_uploading_sub_desc', { defaultValue: 'Keep this window open until the upload finishes.' })}
                        </Typography>
                    </Space>
                );
            },
    ...
    const openUploadSuccessDialog = useCallback(() => {
        const UploadSuccessContent = ({ onClose }: { onClose?: () => void }) => {
            return (
                <Space direction="vertical" size={8} style={{ padding: '4px 0' }}>
                    <Typography style={{ color: 'var(--color-light-6)' }}>
                        {t('crm_import_upload_success_body', {
                            defaultValue:
                                'We’re processing your file for import. You can return to this databoard and check the progress anytime.',
                        })}
                    </Typography>
                </Space>
            );
        ...
        dialog({
            title: t('crm_import_upload_success_title', { defaultValue: 'Upload complete' }),
            showCloseButton: true,
            hideCancelButton: true,
            hideConfirmButton: true,
            actionsAlign: 'flex-start',
            content: ({ onClose }) => {
                return <UploadSuccessContent onClose={onClose} />;
            },
        });
    }, [dialog, t]);
    ...
    const getFileUrl = async (file: File) => {
        ...
        try {
            ...
            // Always close the uploading dialog (success or failure)
            uploadDialogCloseRef.current?.();
            uploadDialogCloseRef.current = null;
            // On success, show a short follow-up dialog (only if we have the uploaded url)
            if (uploadedFileUrlRef.current) {
    ...
    const importFile = useCallback(
        async (file: File | string) => {
            try {
                ...
                setTimeout(() => {
                    console.log('databoard-import-started', id);
                    // Trigger an immediate import_progress check (Databoards page listens to this to restart polling if needed)
                    window.dispatchEvent(new CustomEvent('databoard-import-started', { detail: { boardId: id } }));
                }, 2000);
                ...
                if (data.results.successful + data.results.skipped === data.results.total) {
                    ...
                    const message =
                        data.results.skipped > 0
                            ? `Import data successfully with ${data.results.skipped} data skipped`
                            : t('crm_import_file_mapped_success');
                    notify({
                        type: 'success',
                        message: message,
                    });
