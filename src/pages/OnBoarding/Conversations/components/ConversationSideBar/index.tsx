...
import styles from '@/pages/Conversations/components/ConversationSideBar/index.module.scss';
...
const ConversationSideBar: FC<ConversationSideBarProps> = (props) => {
    ...
    const viewFilterTeamId = 't_imbrace_default_team';
    const viewFilter = 'team' as keyof API.TeamConversationViewCount;
    ...
    const data = useMemo(() => {
        return {
            ...
            soon_to_be: 0,
            overdue: 0,
            t_imbrace_default_team: 1,
        } as API.TeamConversationViewCount;
    }, []);
                    ...
                    <SideBar title={t('conversation_heading')} withBorder={false}>
                        ...
                        <List dense={true} sx={{ padding: 0, mt: '95px' }}>
                            ...
                            <Collapse in={isTeamsBarOpen} timeout="auto" unmountOnExit>
                                <List dense={true} sx={{ padding: 0 }} className={styles.conversationStatusNav}>
                                    <ListItemButton
                                        onClick={() => dispatch(updateViewFilter({ view: 'team', teamId: 't_imbrace_default_team' }))}
                                        isActive={viewFilter === 'team' && viewFilterTeamId === 't_imbrace_default_team'}
                                        sx={{ paddingRight: '41px' }}
                                    >
                                        <ChildListItemText>
                                            <EllipsisText text={'General'} />
                                        </ChildListItemText>
                                        <CountText variant="body1" align="left">
                                            {data.t_imbrace_default_team ? data.t_imbrace_default_team : 0}
                                        </CountText>
                                    </ListItemButton>
                                </List>
                            </Collapse>
