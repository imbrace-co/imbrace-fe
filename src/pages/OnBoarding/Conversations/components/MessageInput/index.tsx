...
import styles from '@/pages/Conversations/components/MessageInput/index.module.scss';
import { StyledIconButton } from '@/pages/Conversations/components/MessageInput/styles';
import Variable from '@/pages/Conversations/components/MessageInput/Variable';
...
import { joinTeamConversationThunk, leaveTeamConversationThunk } from '@/redux/slices/teamConversation';
...
const MessageListInput: FC<MessageListInputProps> = (props) => {
    ...
    const { currentDrawer, setCurrentDrawer, messageListBodyRef, mode } = props;
    ...
    const [templateErrorMessage, setTemplateErrorMessage] = useState<string | undefined>('');
        ...
        if (teamConversation) {
            ...
            textArray.forEach((text, index) => {
                if (index === textArray.length - 1) {
                    result.push(
                        <Typography component={'span'} key={`text-${index}`} sx={{ color: 'var(--color-light-8)' }}>
                            {text}
                        </Typography>,
                    );
                    return;
    ...
    const renderDefaultInputArea = () => {
        return (
            <Tooltip
                title={!channelState?.active || channelState?.is_deleted ? t('error_response_invalid_channel') : ''}
                placement="bottom"
                arrow
            >
                <div>
                        ...
                        request={async () => [
                            {
                                ...
                                disabled: joinTeamConversationStatus === 'FETCH_IN_PROGRESS',
                            },
                            {
    ...
    }, [dispatch, resetInput, selectedMessageTemplate, selectedWhatsAppMessageTemplate, setInput]);
    ...
    useEffect(() => {
        ...
        setTemplateErrorMessage(undefined);
        if ((selectedMessageTemplate && templateHasVariable) || selectedWhatsAppMessageTemplate) {
            if (Object.entries(templateVariable).filter(([, value]) => !value).length !== 0) {
                setTemplateErrorMessage(t('validation_variable_input'));
                for (const key of Object.keys(templateVariable)) {
                    const found = useManualKeyArray.find((element) => element.toString() === key);
                    if (found !== undefined) {
                        setTemplateErrorMessage(t('validation_manual_input'));
                        break;
                    } else if (templateVariable[key]?.length === 0 || templateVariable[key] === undefined) {
                        break;
                    }
                }
            } else {
        ...
        templateVariable,
        setInput,
        selectedWhatsAppMessageTemplate,
        t,
        ...
        <div className={styles.messageListInputRoot}>
            ...
            <div className={`${styles.messageListInputToolBarRoot} ${mode === CONVERSATION_MODE.AUTOMATION && styles.messageToolDisable}`}>
                ...
                <div className={styles.sendContainer}>
                    {!templateErrorMessage && isEmailMode && (
                        <EllipsisText
                            style={{
                    ...
                    {postMessageStatus === 'FETCH_IN_PROGRESS' ? (
                        <div className={styles.messageSendLoadingRoot}>
