...
const Credentials = () => {
    ...
    const src = useMemo(() => {
        ...
        return `${workflowDomain}/credentials/list?lang=${i18n.language ?? 'en'}&token=${token}&organizationId=${organizationId}&userId=${userId}`;
    }, [organizationPartition, i18n, organizationId, userId]);
    

    return <iframe src={src} frameBorder="0" allow="clipboard-read; clipboard-write" title="workflow credentials iframe" style={{ width: '100%', height: '100%' }} />;
};
