...
const getFilteredPanelItems = (boardType: string): string[] => {
    ...
    // Map localStorage keys to panel item names
    const panelItemMapping: Record<string, string> = {
    ...
    Object.entries(panelItemMapping).forEach(([settingKey, panelItemName]) => {
        if (panelSettings[settingKey]) {
            enabledPanelItems.push(panelItemName);
        }
    });
