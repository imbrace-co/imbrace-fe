...
import { attachmentSupportedFileTypes, FieldTypeIcon } from '../../utils';
    ...
    return (
        <Space size={12} align="center">
            ...
            <Controller
                ...
                render={({ field }) => (
                    <FieldSelect
                        queryKey={['dataBoard', 'filters', `${index}`, 'fields']}
                        {...field}
                        value={field.value as string}
                        formControlSx={{
                            width: 180,
            ...
            <IconButton variant="text" size="xs" type="secondary" sx={{ marginLeft: '2px' }} onClick={onDelete}>
                <Icon name="delete" />
            </IconButton>
        </Space>
    );
...
export const FilterPopover = ({
    ...
    saveAsSegment: (formData: FilterFormValue, handleClose: () => void) => void;
}) => {
    const { t } = useTranslation();
        ...
        >
            <div className={styles.filterContainer}>
                <Space size={24} direction="vertical" align="start">
                    <Scrollbars autoHeight autoHeightMax={460} autoHide hideTracksWhenNotNeeded>
                        <Space size={8} direction="vertical" align="start" style={{ width: '100%', padding: '0 32px' }}>
                            ...
                            <Space size={24}>
                                ...
                                <Button
                                    size="xxs"
                                    variant="link"
                                    startIcon={<Icon name="personSharp" />}
                                    text={t('save_as_segment')}
                                    onClick={() => {
                                        saveAsSegment(getValues(), handleClose);
                                    }}
                                    sx={{
                                        textTransform: 'initial',
                                        fontWeight: 400,
                    ...
                    <Space justify="end" style={{ width: '100%', padding: '0 32px' }}>
                        <Button
                            size="xxs"
                            variant="text"
                            text={t('clear_all')}
                        ...
                        />
                        <Button size="xxs" text={t('apply')} onClick={handleSubmit(applyFilter)} />
                    </Space>
                </Space>
            </div>
        </Popover>
...
interface FilterPopoverHOCProps {
    anchorEl: HTMLElement;
    fields?: API.BoardField[];
    currentFilterState?: ColumnFiltersState;
    onFilter: (value?: API.Filters[]) => Promise<void>;
...
const FilterPopoverHOC = ({ onClose, anchorEl, ...restProps }: FilterPopoverHOCProps) => {
...
interface FilterPopoversRef {
    openFilterPopover: (props: FilterPopoverHOCProps) => void;
}
type FilterPopoversItems = FilterPopoverHOCProps & {
    key: string;
};

type FilterPopoversAPI = {
    openFilterPopover: (props: FilterPopoverHOCProps) => void;
};

export const FilterPopovers = forwardRef((props: FilterPopoversProps, ref: ForwardedRef<FilterPopoversRef>) => {
    const [items, setItems] = useState<FilterPopoversItems[]>([]);

    const onClose = (key: string) => {
        setItems((prev) => prev.filter((item) => item.key !== key));
    };
    useImperativeHandle(ref, () => ({
        openFilterPopover: (popoverProps: FilterPopoverHOCProps) => {
            const key = uniqueId('databoard-custom-filter-popover');
            setItems((prev) => {
                const clone = [...prev];
                clone.push({
                    key,
    ...
    return createPortal(
        <>
            {items.map((item) => {
                ...
                return (
                    <FilterPopoverHOC
                        key={`databoard-custom-filter-popover-${key}`}
                        {...restProps}
                        onClose={() => {
                            restProps.onClose?.();
                            onClose(key);
...
export const useFilterPopover = (props?: FilterPopoversProps) => {
    const filterPopoversRef = useRef<FilterPopoversRef>(null);

    const contextHolder = useMemo(() => <FilterPopovers ref={filterPopoversRef} {...props} />, [props]);

    const api = useMemo<FilterPopoversAPI>(
        () => ({
            openFilterPopover: (filterPopoverProps: FilterPopoverHOCProps) => {
    ...
    return [api, contextHolder] satisfies [FilterPopoversAPI, ReactElement];
};
