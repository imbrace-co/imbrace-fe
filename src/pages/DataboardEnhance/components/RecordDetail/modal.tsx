                        ...
                        {fields.map((field) => {
                            ...
                            return (
                                <Space
                                    key={_id}
                                    size={16}
                                    align={type === 'LongText' || type === 'Notes' ? 'start' : 'center'}
                                    style={{ width: '100%' }}
                                >
                                    <Space
                                        size={8}
                                        style={{
                                            ...
                                            fontSize: 20,
                                            ...((type === 'LongText' || type === 'Notes') && { marginTop: '12px' }),
                                        }}
                                    >
                                        {FieldTypeIcon(type)}
...
const RecordDetailModal = (props: RecordDetailModalProps) => {
    const { onClose, recordId, board, tableRef, initialValue, boardData, isEdit = false } = props;
    ...
    const onSubmit = async (data: Record<string, API.RecordValue>) => {
        try {
            ...
            if (updatedRecord?.id && updatedRecord?.id !== controlledRecordId) {
                setControlledRecordId(updatedRecord.id);
            }
