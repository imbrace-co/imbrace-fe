...
import { clearTeamConversation, selectConversation } from '@/redux/slices/teamConversation';
...
const normalizeTeamsMap = (
    teams?: { [key: string]: API.Team } | API.Team[],
): { [key: string]: API.Team } => {
    if (!teams) return {};
    if (Array.isArray(teams)) {
        return teams.reduce<{ [key: string]: API.Team }>((acc, team) => {
...
const serializeTeamConversation = (
    ...
    const teamsMap = normalizeTeamsMap(teams);
    return teamConvs.map((teamConv) => {
        if (teamConv.conversation) {
            ...
            return {
                ...
                is_agent_joined,
                channel_type,
                team_name: teamsMap[teamConv.team_id]?.name || teamConv.team_name,
            };
        }
        return {
            ...teamConv,
...
const ConversationList: FC = () => {
    ...
    // A conversation selected under the previous view/team filter must not stay open when the
    // filter changes. The ref is initialized to the current filter so this does not override a
    // deep-linked conversation on first mount; only later, real filter changes trigger it.
    const filterKey = `${viewFilter}::${viewFilterTeamId ?? ''}`;
    const prevFilterKeyRef = useRef(filterKey);
    const pendingAutoSelectRef = useRef(false);

    // Step 1: as soon as the filter changes, drop the stale selection immediately so the chat
    // pane can never keep showing a conversation from the previous filter (even while the new
    // list is still loading, or when the new filter has no conversations at all).
    useEffect(() => {
    ...
    // Step 2: once the new list has finished loading, auto-select its first conversation. If the
    // new filter is empty, nothing is selected and the pane stays empty (cleared in step 1).
    useEffect(() => {
        if (!pendingAutoSelectRef.current || isFetching) {
            return;
        }
