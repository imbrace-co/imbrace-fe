...
const CommentDetail = forwardRef<CommentEditDetailSave, CommentEditDetailType>((props, ref) => {
    ...
    const resolveLabels = useCallback(
        (raw: unknown): API.TeamConversationLabel[] => {
            if (!Array.isArray(raw)) return [];
            return raw
                .map((entry) => {
                    if (!entry) return null;
                    if (typeof entry === 'string') {
                        return teamLabels.find((l) => l._id === entry) ?? null;
                    }
                    if (typeof entry === 'object' && '_id' in entry) {
                        return entry as API.TeamConversationLabel;
                    }
                    return null;
                })
                .filter((l): l is API.TeamConversationLabel => l !== null);
        },
        [teamLabels],
    );
    ...
    const {
        ...
        defaultValues: {
            ...
            labels: resolveLabels(commentData?.labels),
        },
    });
    ...
    useEffect(() => {
        if (comment?.comments) {
            const commentData = comment?.comments?.find((comment) => comment._id === commentId);
            reset({ content: commentData?.content || '', labels: resolveLabels(commentData?.labels) });
        }
    }, [comment, reset, resolveLabels]);
            ...
            if ((formData.content && formData.content.length > 0) || (formData.labels && formData.labels.length > 0)) {
                const labelsIds = [
                    ...Array.from(
                        new Set(
                            (formData.labels ?? [])
                                .filter((l): l is API.TeamConversationLabel => Boolean(l?._id))
                                .map(({ _id }) => _id),
                        ),
                    ),
                ];
