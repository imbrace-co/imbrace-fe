...
const StatusI18n = {
    active: 'conversation_status_online',
            ...
            switch (latestMessage.type) {
                ...
                case 'whatsapp.template':
                    if (typeof (latestMessage.content.text as unknown) !== 'string') {
                        return <EllipsisText style={{ color: 'var(--color-light-6)' }} text={getSafeContentText(latestMessage.content.text as unknown)} />;
                    }
                    ...
                    primary={
                        <div className={styles.titleContainer}>
                            ...
                            <div className={styles.timestamp}>
                                {/* TODO: render status */}
                                {/* {channelType === 'web' && isPresence ? <OnlineStatus isOnline={isPresence} /> : renderTimestamp()} */}
                                {/*{unreadTeamConvs.includes(conversationId) && <div className={styles.unreadDot} />}*/}
                                {status && <StatusDot text={t(StatusI18n[status])} statusType={status} />}
                            </div>
                        </div>
                    }
                    secondary={
