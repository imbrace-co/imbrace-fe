import { CopyButton, EllipsisText, Icon, Space, Tooltip, Typography } from '@imbrace/ui';
...
const MessageListHeader: FC<MessageListHeaderProps> = (props) => {
    ...
    const conversationLink = (conversationId: string) => {
        const origin = (env.VITE_APP_HOST || window.location.origin).replace(/\/+$/, '');
        return `${origin}/chatroom?to_conv=${conversationId}`;
    };
            ...
            {!isEmpty(teamConversation) ? (
                <Space size={10} style={{ width: '100%', flex: 1 }}>
                    ...
                    <div className={styles.extra}>
                        ...
                        <Divider
                            sx={{ height: 24, borderColor: '#e0e0e0', alignSelf: 'center', display: 'none' }}
                            orientation="vertical"
                            variant="middle"
                            flexItem
                        />
                        {/* Hidden: invite team - not deleted, may be re-enabled later */}
