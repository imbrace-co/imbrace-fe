...
const FlexibleTable = <D extends { id: string; _id?: string }>(props: FlexibleTableProps<D>, ref: ForwardedRef<FlexibleTableRef<D>>) => {
    ...
    const { data, isFetching, refetch } = useQuery({
        ...
        queryFn: async ({ queryKey, signal }) => {
            ...
            if ('dataSource' in props) {
                if (refInitialData.current.length === 0 || refInitialData.current.length !== props.dataSource.length) {
                    refInitialData.current = JSON.parse(JSON.stringify(props.dataSource));
                }
                return {
    ...
    const columnVirtualizer = useVirtualizer({
        ...
        rangeExtractor: useCallback(
            (range: Range) => {
                const pinnedIndexes = [...Array(lastPinnedColumnIndex + 1).keys()];
                const next = new Set([...pinnedIndexes, ...defaultRangeExtractor(range)]);
                return [...next];
            },
    ...
    useEffect(() => {
        ...
        const columnPinningFiltered = columns.filter((col) => col.meta?.identifier ?? col.enablePinning);

        const pinning = columnPinningFiltered.filter((col) => col.enablePinning);
        const identifiers = columnPinningFiltered.filter((col) => col.meta?.identifier);

        const orderedCols = [...pinning, ...identifiers];

        const columnPinningLeft = orderedCols.map((col) => col.accessorKey) as string[];
    ...
    const cellWidths = table.getHeaderGroups()[0]?.headers?.map((header) => {
        const metaWidth = (header.column.columnDef.meta as any)?.width;
        if (typeof metaWidth === 'string') {
            return metaWidth;
        }
    ...
    const hasResponsiveWidth = useMemo(
        () => cellWidths.some((w) => typeof w === 'string' && (w.includes('%') || w.includes('fr'))),
        [cellWidths],
    );
    ...
    if (columnOrderChangeable) {
        return (
            <Space
                ...
                direction="vertical"
                style={{
                    // The table sits inside .app (height 100vh - --message-bar-height) but this
                    // calc uses raw 100vh, so subtract the same var — otherwise the table runs 54px
                    // too tall whenever the announcement bar is shown, pushing pagination off-screen.
                    // (No-bar rendering is unchanged: the var resolves to 0px when no bar is active.)
                    height: `calc(100vh - ${table.getAllColumns().length === 0 ? '140px' : '270px'} - var(--message-bar-height, 0px))`,
                    width: '100%',
                    position: 'relative',
                    ...containerStyle,
                }}
                ...
                >
                    ...
                    <div className={`${styles.tableContainer} ${data?.data.length === 0 ? styles.empty : ''}`}>
                        <div className={`${isFetching ? styles.blur : ''}`}>
                            <div
                                className={`${styles.flexibleTable} ${headerPaddingModify ? styles.headerSmallPadding : ''} ${
                                    tableRowNoPadding ? styles.tableRowNoPadding : ''
                                } ${dataRowNoPadding ? styles.dataRowNoPadding : ''}`}
                                style={{
                                    width: hasResponsiveWidth ? '100%' : `${table.getTotalSize()}px`,
                                }}
                            >
                                {renderThead()}
                                {renderTbody()}
