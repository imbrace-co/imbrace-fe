...
const Row = <D extends { id: string }>({
    ...
    return (
        <div
            ...
            data-index={index}
            className={`${styles.row} ${index % 2 !== 0 && !transparentRow ? styles.evenBackground : ''} ${row.getIsSelected() || row.original.id === 'new' ? styles.selected : ''} ${disableHoverEffect ? styles.disabledHoverEffect : ''
                }`}
            style={gridStyles}
            onMouseEnter={() => {
                setIsHover(true);
                        ...
                        style={
                            cellColumn?.getIsPinned()
                                ? {
                                    ...
                                    backgroundColor: index % 2 !== 0 && !transparentRow ? '#F2F2F2' : transparentRow ? 'transparent' : 'white'
                                }
                                : {
                                    gridArea: `column-${virtualColumn.index + 1}`,
                                }
