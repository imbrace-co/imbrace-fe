...
const Attachment = ({
    ...
    const sanitizeExtra = (extra: AttachmentValue['extra']) => {
        if (!extra) return extra;
        const { file, ...rest } = extra as AttachmentType & { file?: unknown };
        return file instanceof Blob ? { ...rest, file } : rest;
    };
    ...
    const [attachments, setAttachments] = useState<AttachmentType[]>(
        Array.isArray(value)
            ? (value as AttachmentValue[])?.map((attachment, index) => ({
                  ...
                  ...sanitizeExtra(attachment?.extra),
              }))
            : [],
    );
    ...
    return (
        <Upload
            ...
            value={attachments}
            hideAddButton
            isLocalStorage={env.VITE_APP_ON_PREMISE || false}
