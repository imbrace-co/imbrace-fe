...
const EditableColumn = <D extends { id: string; [key: string]: string | number | (string | number)[] }>(
    ...
    const isInvalidFormat =
        ...
        initialValue.some((v: any) => v !== null && v !== undefined && typeof v !== 'string');
    ...
    const renderFormattedValue = () => {
        ...
        if (column.enum) {
            if (Array.isArray(initialValue)) {
                if (column.type === 'MultipleSelection') {
                    if (isInvalidFormat) {
                        return <Typography style={{ color: 'var(--color-danger-1)' }}>Wrong format</Typography>;
                    }
                                            ...
                                            <Chip
                                                ...
                                                label={
                                                    typeof enumValue === 'object'
                                                        ? (enumValue as any)?.name ||
                                                          (enumValue as any)?.label ||
                                                          (enumValue as any)?.display_name ||
                                                          (enumValue as any)?.value ||
                                                          `${enumValue}`
                                                        : (column.enum?.[`${enumValue}`] ?? (enumValue as string))
                                                }
                                            />
                                        );
                                    })}
        ...
        if (column.type === 'Currency') {
            ...
            return (
                <EllipsisText
                    text={`${
                        currencyData?.currency_code || column.meta?.defaultCurrencyCode
                            ? Intl.NumberFormat('en-US', {
                                  style: 'currency',
                                  currency: currencyData?.currency_code || column.meta?.defaultCurrencyCode,
