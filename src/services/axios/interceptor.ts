...
export const responseErrorInterceptor = (
    error: AxiosError<{ code: number | string; message: string | { message: string } }, Record<string, unknown>>,
) => {
    /**
     * Sentry fingerprint
     */
    ...
    if (error?.response?.data?.code === 99999) {
        if (error.config.url?.indexOf('meilisearch') === -1 && error.config.url?.indexOf('data-board/search') === -1) {
            let message;
    ...
    if (error?.response?.data?.code === 'LICENSE_INVALID') {
        navigate('/license');
    }
